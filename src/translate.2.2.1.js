...
;(function (factory) {
    ...
    /**--------------------**/
    var rootConfig = {
        rootScope: window,//用于将插件查询作用域从window对象下更改到指定对象作用域
        scopeCamp: {
            // 'translate': {
            //     scope: Object,
            //     scopeString: "trans"
        ...
        translateWrapperHandleFlag: 'wrapperHandled'
    };
    ...
    var i18n = function (TSetting) {
        ...
        self.currentLanguageTag = TSetting["default"];
        ...
        //注册当前域,并检测是否已被注册过
        if (rootConfig.namespaceCamp.join(",").indexOf(self.namespace) == -1) {
            __arry__extends(rootConfig.namespaceCamp,[self.namespace]);
        } else {
            throw Error ("The namespace ["+self.namespace+"] has already been registered by {"+rootConfig.scopeCamp[self.namespace].scopePath+"},can not be used again!")
        };
        /**--------------------**/
        /********根域数据注册*********/
        /**--------------------**/
        ...
        var onDomInsert = function () {
                ...
                var watchTarget = window.document.querySelector(typeof argus[0] == "string" ? (callback = argus[1], argus[0]) : "body");
        ...
        __attr__extends(Modules.textController,{
            ...
            innerTextCompiler: function (targetString,cb) {
                ...
                if (targetString && targetString != ""){
                    ...
                    // var wrappers = Modules.commonMethod.getAllTargetNodes('[' + rootConfig.translateWrapper + ']');
        ...
        __attr__extends(Modules.nodesController,{
            ...
            selectTargetToRegTranslator: function (range) {
                ...
                newTargets = newTargets.filter(function (ele) {
                    //去掉已注册过的元素
                    if (ele[rootConfig.elementRegisteredTagHead + self.namespace]) {
                        return false;
                    } else {
                        return true;
                    }
                ...
                newTargets.forEach(function (ele, i) {
                    //翻译当前域标签内容
                    var transSource = ele.getAttribute(self.namespace)
                    //验证当前是否已经注册了语言数据
                    if (self.currentLanguageData && transSource) {
                ...
                newTargets = newTargets.filter(function (ele) {
                    ...
                    rootConfig.namespaceCamp.forEach(function (n) {
                        if (n != self.namespace) foreignSpace.push(n);
                    });
    ...
    var useRegisteredNamespace = function (NS) {
        return rootConfig.scopesContainer[NS]
            ? rootConfig.scopesContainer[NS]
